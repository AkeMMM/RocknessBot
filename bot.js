...
client.on("message", async message => {
  ...
  if (message.content.length > 2) {
    ...
    client.on("message", msg => {
      if (msg.content === "!yayınhakan") {
        msg.reply("https://www.twitch.tv/rockness Yayındayız");
      }
    });

    client.on("message", msg => {
      if (msg.content === "!yayınadil") {
        msg.reply("https://www.twitch.tv/arawn75 Yayındayız");
      }
    });
...
client.on("guildMemberAdd", async member => {
  ...
  if (cezalımı == "cezali") {
    ...
    member.send(
      "Cezalıyken Sunucudan Çıktığın için Yeniden Cezalı Rolü Verildi, Beni mi Kandıracaktın Aptal!"
    );
    setTimeout(function() {
      // msg.channel.send(`<@${user.id}> Muten açıldı.`)
